...
// error_log("[Sync-Ajax] Request received: " . json_encode($_POST));

// if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['sync_customers'])) {
//     error_log("[Sync-Ajax] Invalid request - missing sync_customers param");
//     header('Content-Type: application/json');
...
// error_log("[Sync-Ajax] shop=$shop, segment=$segment_id, local=$local_segment_id");

// if (!$shop || !$segment_id || !$local_segment_id) {
//     error_log("[Sync-Ajax] Missing required parameters");
//     header('Content-Type: application/json');
...
// $pdo = getDatabaseConnection();
// if (!$pdo) {
//     error_log("[Sync-Ajax] Database connection failed");
//     header('Content-Type: application/json');
...
// if (!$tokenData) {
//     error_log("[Sync-Ajax] Shop not found: $shop");
//     $failPdo = getDatabaseConnection();
//     $stmt = $failPdo->prepare("UPDATE $customerSegmentTable SET sync_status = 'failed' WHERE id = ?");
//     $stmt->execute(array($local_segment_id));
//     $failPdo = null;
...
//         error_log("[Sync-Ajax] API call - HTTP: $httpCode");

//         if (curl_errno($ch) || $httpCode >= 400) {
//             $curlError = curl_error($ch);
//             curl_close($ch);
//             throw new Exception("API request failed: HTTP " . $httpCode . " - " . $curlError);
//         }
...
//     error_log("[Sync-Ajax] Fetched " . count($allCustomers) . " customers");
...
//     error_log("[Sync-Complete] Background sync completed for segment $local_segment_id: $totalInserted customers synced");
...
//     $stmt->execute(array($local_segment_id));
//     $errorPdo = null;
//     error_log("[Sync-Error] Background sync failed for segment $local_segment_id: " . $e->getMessage());
// }