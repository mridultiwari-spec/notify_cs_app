...
$aid = isset($_GET['aid']) ? (int) $_GET['aid'] : (isset($_POST['aid']) ? (int) $_POST['aid'] : 5);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $timeinterval = isset($_POST['timeinterval']) ? (int) $_POST['timeinterval'] : 0;
            ...
            if ($exists) {
                $stmt = $pdo->prepare("
                    ...
                    SET sms = :sms, template_name = :template_name, sms_variables = :sms_variables,
                        discount_code = :discount_code, order_condition = :order_condition, timeinterval = :timeinterval
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO `$table` (aid, sms, shop, template_name, sms_variables, discount_code, order_condition, timeinterval)
                    VALUES (:aid, :sms, :shop, :template_name, :sms_variables, :discount_code, :order_condition, :timeinterval)
                ");
                $stmt->execute(array(
                    ':aid' => $aid,
                    ':sms' => $sms,
        ...
        if (isset($_POST['media_type'])) {
            ...
            if ($exists) {
                ...
                $stmt = $pdo->prepare("INSERT INTO `$table` (
                    ...
                    discount_code, order_condition, timeinterval
                ) VALUES (
                    :aid, :shop, :media_type, :media_url, :media_source,
...
<body>
    <div class="container-box">
        <div class="page-header">
            <div class="header-left">
                ...
                <div class="page-title">Abandoned Checkout</div>
            </div>
        </div>
            ...
            <div class="left-panel">
                ...
                <form method="POST" action="">
                    ...
                    <div class="tab-content active" id="sms">
                        ...
                        <div class="condition-box">
                            <span>Condition : </span>
                            <select class="condition-select" name="order_condition">
                                <option value="Number of hours" <?php echo ($isEditMode && isset($existingData['order_condition']) && $existingData['order_condition'] == 'Number of hours') ? 'selected' : ''; ?>>Number of hours</option>
                                <option value="Number of days" <?php echo ($isEditMode && isset($existingData['order_condition']) && $existingData['order_condition'] == 'Number of days') ? 'selected' : ''; ?>>Number of days</option>
                                <option value="Number of weeks" <?php echo ($isEditMode && isset($existingData['order_condition']) && $existingData['order_condition'] == 'Number of weeks') ? 'selected' : ''; ?>>Number of weeks</option>
                            </select>
                            <input type="number" class="condition-input" placeholder="Enter number" name="timeinterval"
                                value="<?php echo $isEditMode && isset($existingData['timeinterval']) ? htmlspecialchars($existingData['timeinterval'], ENT_QUOTES, 'UTF-8') : ''; ?>">
                        </div>
                        <div class="condition-box">
                            <span>Enter Discount Code : </span>
                            <input type="text" class="condition-input" placeholder="Discount code" name="discount_code"
                                value="<?php echo $isEditMode && isset($existingData['discount_code']) ? htmlspecialchars($existingData['discount_code'], ENT_QUOTES, 'UTF-8') : ''; ?>">
                        </div>
                        <div class="btn-group">
            ...
            <div class="right-panel">
                <div class="variable-box">
                    ...
                    <div class="variable-list">
                        <div class="variable-item">{{ product_name }}</div>
                        <div class="variable-item">{{ total_price }}</div>
                        <div class="variable-item">{{ checkout_created_at }}</div>
                        <div class="variable-item">{{ customer_fname }}</div>
                        <div class="variable-item">{{ customer_lname }}</div>
                        <div class="variable-item">{{ customer_email }}</div>
                        <div class="variable-item">{{ customer_phone }}</div>
                        <div class="variable-item">{{ country_code }}</div>
                        <div class="variable-item">{{ checkout_url }}</div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            var existingButtons = document.querySelectorAll('#buttons-container .button-group');
            buttonCount = existingButtons.length;
            if (buttonCount === 0) {
                addButton();
            } else {
        ...
        var headerCounter = 1;
        var bodyCounter = 1;
        var existingHeaders = document.querySelectorAll('#headers-container .variable-row');
        ...
        function removeButton(buttonNumber) {
            ...
            if (buttonGroup) {
                buttonGroup.remove();
                buttonCount--;
                var remainingButtons = document.querySelectorAll('#buttons-container .button-group');
                for (var i = 0; i < remainingButtons.length; i++) {
                    var newNumber = i + 1;
                    var group = remainingButtons[i];
                    group.id = 'button-group-' + newNumber;
                    var titleSpan = group.querySelector('.button-title');
                    if (titleSpan) titleSpan.textContent = 'Button ' + newNumber;
                    var removeBtn = group.querySelector('.remove-button-btn');
                    if (removeBtn) removeBtn.setAttribute('onclick', 'removeButton(' + newNumber + ')');
                    var select = group.querySelector('.button-type');
                    if (select) {
                        select.setAttribute('name', 'button_type' + newNumber);
                        select.setAttribute('data-button', newNumber);
                        select.setAttribute('onchange', 'toggleUrlField(' + newNumber + ')');
                    }
                    var textInput = group.querySelector('.button-row input');
                    if (textInput) textInput.setAttribute('name', 'button_text' + newNumber);
                    var urlField = group.querySelector('.url-field');
                    if (urlField) {
                        urlField.id = 'url-field-' + newNumber;
                        var urlInput = urlField.querySelector('input');
                        if (urlInput) urlInput.setAttribute('name', 'button_url' + newNumber);
                    }
                }
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            for (var i = 0; i < variableItems.length; i++) {
                variableItems[i].addEventListener('click', function () {
                    ...
                    navigator.clipboard.writeText(text)
                        .then(function () { shopify.toast.show('Variable copied to clipboard', { duration: 3000 }); })
                        .catch(function () { shopify.toast.show('Failed to copy', { isError: true, duration: 3000 }); });
                });
            }
            ...
            function toggleMediaFields() {
                ...
                for (var i = 0; i < radioButtons.length; i++) {
                    if (radioButtons[i].checked) { selected = radioButtons[i]; break; }
                }
                if (!selected && mediaType.value !== 'text') {
                    for (var i = 0; i < radioButtons.length; i++) {
                        if (radioButtons[i].value === 'url') { radioButtons[i].checked = true; selected = radioButtons[i]; break; }
                    }
                }
                if (!selected) {
                ...
                if (selected.value === 'url') { urlBox.style.display = 'flex'; fileBox.style.display = 'none'; }
                else if (selected.value === 'file') { urlBox.style.display = 'none'; fileBox.style.display = 'flex'; }
                else if (selected.value === 'dynamic') { urlBox.style.display = 'none'; fileBox.style.display = 'none'; }
            }

            for (var i = 0; i < radioButtons.length; i++) radioButtons[i].addEventListener('change', toggleMediaFields);
            mediaType.addEventListener('change', toggleMediaFields);
            toggleMediaFields();
            for (var i = 1; i <= buttonCount; i++) toggleUrlField(i);
        });
    </script>
