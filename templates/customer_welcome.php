...
$aid = isset($_GET['aid']) ? (int) $_GET['aid'] : (isset($_POST['aid']) ? (int) $_POST['aid'] : 10);
    ...
    try {
        ...
        if (isset($_POST['sms'])) {
            ...
            send_json_response(array(
                ...
                'redirect_url' => $app_url . "/index.php?tab=customer&shop=" . urlencode($shop),
...
<body>
        ...
        <div class="page-header">
            <div class="header-left">
                <a class="material-symbols-outlined"
                    href="<?php echo htmlspecialchars($app_url, ENT_QUOTES, 'UTF-8'); ?>?tab=customer&shop=<?php echo $shop; ?>">arrow_back</a>
                <div class="page-title">Customer Welcome</div>
            </div>
        </div>
    ...
    <script>
        ...
        $btnCount = 0;
        if ($isEditMode) {
            $button1Exists = isset($existingData['button_type']) && !empty($existingData['button_type']);
            $button2Exists = isset($existingData['button_type2']) && !empty($existingData['button_type2']);
            $button3Exists = isset($existingData['button_type3']) && !empty($existingData['button_type3']);
        ...
        function addButton() {
            var existingButtons = document.querySelectorAll('.button-group');
            var currentButtonCount = existingButtons.length;
            ...
            var newButtonNumber = currentButtonCount + 1;

            while (document.getElementById('button-group-' + newButtonNumber)) {
                newButtonNumber++;
            ...
            newButtonGroup.id = 'button-group-' + newButtonNumber;
            newButtonGroup.innerHTML = '<div class="button-header">' +
                '<span class="button-title">Button ' + newButtonNumber + '</span>' +
                '<button type="button" class="remove-button-btn" onclick="removeButton(' + newButtonNumber + ')">' +
                ...
                '</svg></span></button></div>' +
                '<div class="button-row">' +
                '<select name="button_type' + newButtonNumber + '" class="button-type" data-button="' + newButtonNumber + '" onchange="toggleUrlField(' + newButtonNumber + ')">' +
                '<option value="none">None</option>' +
                '<option value="cta">CTA</option>' +
                '<option value="quick">Quick Reply</option>' +
                '</select>' +
                '<input type="text" name="button_text' + newButtonNumber + '" placeholder="Button Text">' +
                '</div>' +
                '<div class="url-field" id="url-field-' + newButtonNumber + '">' +
                '<input type="text" name="button_url' + newButtonNumber + '" placeholder="Enter URL (e.g., https://example.com)">' +
                '</div>';
            container.appendChild(newButtonGroup);

            buttonCount = newButtonNumber;
        }
        ...
        function removeButton(buttonNumber) {
            ...
            if (buttonGroup) {
                buttonGroup.remove();
                var remainingButtons = document.querySelectorAll('.button-group');
                buttonCount = remainingButtons.length;
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function () {
                    ...
                    xhr.onreadystatechange = function () {
                        ...
                        if (xhr.status === 200) {
                            ...
                            if (data && data.success) {
                                ...
                                setTimeout(function () {
                                    var redirectUrl = data.redirect_url ? data.redirect_url : "<?php echo htmlspecialchars($app_url, ENT_QUOTES, 'UTF-8'); ?>/index.php?tab=customer&shop=<?php echo urlencode($shop); ?>";
            ...
            var existingButtonGroups = document.querySelectorAll('.button-group');
            for (var i = 0; i < existingButtonGroups.length; i++) {
                var btnId = existingButtonGroups[i].id.replace('button-group-', '');
