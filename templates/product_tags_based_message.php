...
if (isset($_GET['edit_id'])) {
    $id = $_GET['edit_id'];
    $stmt = $pdo->prepare("
        SELECT * FROM $table WHERE id = :id AND shop = :shop
    ");
    $stmt->execute(array(
        ':id' => $id,
        ':shop' => $shop
    ...
    try {
        ...
        if (isset($_POST['sms_tab_submit'])) {
            ...
            if (!empty($id)) {
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO $table 
                    (aid, shop, order_name, order_condition, tags_data, sms, template_name, sms_variables)
                    VALUES 
                    (:aid, :shop, :order_name, :order_condition, :tags_data, :sms, :template_name, :sms_variables)
                ");
                $stmt->execute(array(
            ...
            if (!empty($id)) {
                ...
                $stmt = $pdo->prepare("INSERT INTO $table (
                    aid, shop, order_name, order_condition, tags_data, media_type, media_url, media_source,
...
$stmt = $pdo->prepare("
    SELECT id, order_name, order_condition, tags_data, sms_enabled, whatsapp_enabled 
    FROM $table 
    WHERE shop = :shop AND aid = 12
    ORDER BY id DESC
");
...
<head>
    ...
    <title>Product Tags Based</title>
...
<body>
    <div class="container-box">
        <div class="title-row">
            ...
            <div class="page-title">Promotional Message Based On Product Tags</div>
        </div>
        <div class="header-row">
            <button class="submit-btn" onclick="openModal()">Add Product Tags</button>
        </div>
            ...
            <tbody>
                <?php if (!empty($rows)): ?>
                    <?php foreach ($rows as $row): ?>
                        <tr id="row-<?= $row['id'] ?>">
                            <td style="white-space: nowrap;"><?= htmlspecialchars($row['order_name']) ?></td>
                            <td style="white-space: nowrap;"><?= htmlspecialchars($row['order_condition']) ?></td>
                            <td style="white-space: normal; word-break: break-word; max-width: 300px;">
                                <?= htmlspecialchars($row['tags_data']) ?>
                            </td>
                            ...
                            <td>
                                <button type="button" class="test-btn"
                                    onclick="openTestModal(<?= $row['id'] ?>, '<?= htmlspecialchars($row['order_condition']) ?>')">
                                    <span class="material-symbols-outlined" style="font-size: 13px;">play_arrow</span>
                                    Test trigger
                                </button>
                            </td>
                            <td class="action-cell" style="white-space: nowrap; text-align: right;">
            ...
            <div class="left-panel">
                ...
                <form method="POST" action="" enctype="multipart/form-data" id="smsForm">
                    ...
                    <div class="modal-tab-content active" id="modal-sms">
                        ...
                        <div class="condition-box">
                            ...
                            <input type="text" name="tags_data" id="sms_tags_data">
                        </div>
                ...
                <form method="POST" action="" enctype="multipart/form-data" id="whatsappForm">
                    ...
                    <div class="modal-tab-content" id="modal-whatsapp">
                        ...
                        <div class="condition-box">
                            <span>Tags Value :</span>
                            <input type="text" name="tags_data" id="whatsapp_tags_data">
                        </div>
                        <div class="condition-box">
            ...
            <div class="right-panel">
                <div class="variable-box">
                    ...
                    <div class="variable-list">
                        ...
                        <div class="variable-item">{{ product_tags }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function editRow(id) {
            ...
            fetch(`?edit_id=${id}&shop=<?php echo $shop; ?>`)
                ...
                .then(data => {
                    ...
                    const smsOrderCondition = document.getElementById('sms_order_condition');
                    const smsTagsData = document.getElementById('sms_tags_data');
                    const whatsappOrderName = document.getElementById('whatsapp_order_name');
                    const whatsappOrderCondition = document.getElementById('whatsapp_order_condition');
                    const whatsappTagsData = document.getElementById('whatsapp_tags_data');

                    if (smsOrderName) smsOrderName.value = data.order_name || '';
                    if (smsOrderCondition) smsOrderCondition.value = data.order_condition || 'Tags';
                    if (smsTagsData) smsTagsData.value = data.tags_data || '';
                    if (whatsappOrderName) whatsappOrderName.value = data.order_name || '';
                    if (whatsappOrderCondition) whatsappOrderCondition.value = data.order_condition || 'Tags';
                    if (whatsappTagsData) whatsappTagsData.value = data.tags_data || '';
        ...
        function sendTestFromModal() {
            ...
            fetch('/notifycsapp/test_trigger/test_product_tags_message.php', {
                method: 'POST',
                headers: {
