...
if (isset($_GET['delete_id'])) {
    try {
        ...
        send_json_response(array("error" => $e->getMessage()));
    }
}
...
if ($_SERVER["REQUEST_METHOD"] === 'POST') {
    ...
    try {
        ...
        $aid = isset($_POST['aid']) ? $_POST['aid'] : '11';
        ...
        if (isset($_POST['sms_tab_submit'])) {
            ...
            if (!empty($id)) {
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO $table 
                    (aid, shop, order_name, order_condition, price, sms, template_name, sms_variables)
                    VALUES 
                    (:aid, :shop, :order_name, :order_condition, :price, :sms, :template_name, :sms_variables)
                ");
                $stmt->execute(array(
            ...
            send_json_response(array(
                ...
                'redirect_url' => $app_url . "/templates/order_value_based_message.php?shop=" . urlencode($shop)
            ));
            ...
            if ($media_type !== 'text') {
                if ($media_source_type === 'url') {
                    ...
                    if ($newFileUploaded) {
                        ...
                        // Delete old file if exists (optional) - need to fetch existing data first
                        if ($isEditMode) {
            ...
            if (!empty($id)) {
                ...
                $stmt = $pdo->prepare("INSERT INTO $table (
                    aid, shop, order_name, order_condition, price, media_type, media_url, media_source,
...
$stmt = $pdo->prepare("
    SELECT id, order_name, order_condition, price, sms_enabled, whatsapp_enabled 
    FROM $table 
    WHERE shop = :shop AND aid = 11
    ORDER BY id DESC
");
...
<body>
    <div class="container-box">
        <div class="title-row">
            ...
            <div class="page-title">Promotional Message Based On Order Value</div>
        </div>
        <div class="header-row">
            <button class="submit-btn" onclick="openModal()">Add Orders Value</button>
        </div>
            ...
            <div class="left-panel">
                ...
                <form method="POST" action="" enctype="multipart/form-data" id="smsForm">
                    ...
                    <div class="modal-tab-content active" id="modal-sms">
                        ...
                        <div class="condition-box">
                            <span>Condition :</span>
                            <select name="order_condition" id="sms_order_condition">
                        ...
                        <div class="condition-box">
                            <span>Price :</span>
                            <input type="number" name="price" id="sms_price">
                        </div>
                ...
                <form method="POST" action="" enctype="multipart/form-data" id="whatsappForm">
                    ...
                    <div class="modal-tab-content" id="modal-whatsapp">
                        ...
                        <div class="condition-box">
                            <span>Condition :</span>
                            <select name="order_condition" id="whatsapp_order_condition">
                                <option>Greater than</option>
                                <option>Less than</option>
                                <option>Equals</option>
                                <option>Between</option>
                        ...
                        <div class="condition-box">
                            <span>Price :</span>
                            <input type="number" name="price" id="whatsapp_price">
                        </div>
                        <div class="condition-box">
    ...
    <script>
                    ...
                    if (enabled == 1) {
                        ...
                        xhrCheck.onreadystatechange = function () {
                            if (xhrCheck.readyState === 4 && xhrCheck.status === 200) {
                                ...
                                if (channel === 'sms') {
                                    // Check both SMS content AND template name
        ...
        function resetForms() {
            ...
            document.getElementById('whatsapp_edit_id').value = '';
            currentEditId = null;
            buttonCount = 0;
        ...
        function editRow(id) {
            ...
            fetch(`?edit_id=${id}&shop=<?php echo $shop; ?>`)
                ...
                .then(data => {
                    openModal();
                    document.getElementById('sms_order_name').value = data.order_name || '';
                    document.getElementById('sms_order_condition').value = data.order_condition || '';
                    document.getElementById('sms_price').value = data.price || '';
                    document.getElementById('whatsapp_order_name').value = data.order_name || '';
                    document.getElementById('whatsapp_order_condition').value = data.order_condition || '';
                    document.getElementById('whatsapp_price').value = data.price || '';

                    document.getElementById('edit_id').value = data.id;
                    document.getElementById('whatsapp_edit_id').value = data.id;

                    document.getElementById('sms_content').value = data.sms || '';
                    document.getElementById('sms_template_name').value = data.template_name || '';
                    ...
                    if (data.sms_variables) {
                        ...
                        for (let key in vars) {
                            if (index <= 6) {
                                let keyInput = document.getElementById(`sms_key_${index}`);
                                let varInput = document.getElementById(`sms_var_${index}`);
                    ...
                    document.getElementById('mediaType').value = data.media_type || 'text';
                    document.getElementById('whatsapp_template_name').value = (data.whatsapp && data.whatsapp.template_name) ? data.whatsapp.template_name : '';

                    if (data.media_source === 'file') {
                        let fileRadio = document.querySelector('input[name="media_source_type"][value="file"]');
                        if (fileRadio) fileRadio.checked = true;
                    } else if (data.media_source === 'dynamic') {
                        let dynamicRadio = document.querySelector('input[name="media_source_type"][value="dynamic"]');
                        if (dynamicRadio) dynamicRadio.checked = true;
                    } else {
                        let urlRadio = document.querySelector('input[name="media_source_type"][value="url"]');
                        if (urlRadio) urlRadio.checked = true;
                    }

                    let mediaUrlInput = document.getElementById('media_url_input');
        ...
        document.querySelectorAll('.variable-item').forEach(item => {
            item.addEventListener('click', function () {
                ...
                navigator.clipboard.writeText(text)
                    .then(() => shopify.toast.show("Variable copied to clipboard", { duration: 3000 }))
        ...
        function openTestModal(id, name) {
            ...
            // Set search input to currently selected country
        ...
        function sendTestFromModal() {
            ...
            // ADD THIS CHECK - Verify template is still enabled
            var smsEnabled = false;
            var whatsappEnabled = false;
            ...
            fetch('/notifycsapp/test_trigger/test_order_value_message.php', {
                method: 'POST',
                headers: {
        ...
        function filterCountries() {
            ...
            // Scroll to first visible option
