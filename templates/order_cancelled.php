...
function getExistingTemplate($pdo, $table, $shop, $aid)
{
    $stmt = $pdo->prepare("SELECT * FROM `$table` WHERE shop = :shop AND aid = :aid LIMIT 1");
...
$existingData = getExistingTemplate($pdo, $table, $shop, $aid);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (isset($_POST['sms'])) {
            ...
            $checkStmt = $pdo->prepare("SELECT id FROM `$table` WHERE shop = :shop AND aid = :aid");
            $checkStmt->execute(array(':shop' => $shop, ':aid' => $aid));
        ...
        if (isset($_POST['media_type'])) {
                ...
                if ($media_source_type === 'url') {
                    ...
                    if ($newFileUploaded) {
                        ...
                        if ($isEditMode && $existingMediaSource === 'file' && !empty($existingMediaUrl)) {
                            $oldFileName = basename($existingMediaUrl);
            ...
            if ($exists) {
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO `$table` (
                        aid, shop, media_type, media_url, media_source, 
                        button_type, button_type2, button_type3, cta_url, 
                        button_text1_type, button_text2_type, button_text3_type, whatsapp
                    ) VALUES (
                        :aid, :shop, :media_type, :media_url, :media_source,
                        :button_type1, :button_type2, :button_type3, :cta_url,
                        :button_text1, :button_text2, :button_text3, :whatsapp
                    )
                ");
            }
...
<body>
        ...
        <div class="page-header">
            <div class="header-left">
                <a class="material-symbols-outlined"
                    href="<?php echo htmlspecialchars($app_url, ENT_QUOTES, 'UTF-8'); ?>?tab=orders&shop=<?php echo $shop; ?>">arrow_back</a>
                <div class="page-title">Order Cancelled</div>
            </div>
        </div>
                ...
                <form method="POST" action="">
                    ...
                    <div class="tab-content active" id="sms">
                        <textarea id="smsBox" name="sms" placeholder="Enter SMS template..."
                            style="display:none;"><?php echo isset($existingData['sms']) ? htmlspecialchars($existingData['sms'], ENT_QUOTES, 'UTF-8') : ''; ?></textarea><br>
                        <p></p>
                        ...
                        <input type="text" name="sms_template_name" class="condition-input template-name-input"
                            ...
                            value="<?php echo isset($existingData['template_name']) ? htmlspecialchars($existingData['template_name'], ENT_QUOTES, 'UTF-8') : ''; ?>">
                        <br>
                        <p></p>
                        <span>Variables : </span>
                        <p></p>
                        <?php
                        $smsVariables = array();
                        if (isset($existingData['sms_variables']) && !empty($existingData['sms_variables'])) {
                            $smsVariables = json_decode($existingData['sms_variables'], true);
                            if (!is_array($smsVariables)) {
                                $smsVariables = array();
                            }
                        }
                        $varKeys = array_keys($smsVariables);
                        $varValues = array_values($smsVariables);
                        ?>
                        <?php for ($i = 1; $i <= 6; $i++): ?>
                            <div class="condition-box">
                                <div style="display: flex; gap: 10px; flex: 1;">
                                    <input type="text" name="key_<?php echo $i; ?>" id="sms_key_<?php echo $i; ?>"
                                        ...
                                        value="<?php echo isset($varKeys[$i - 1]) ? htmlspecialchars($varKeys[$i - 1], ENT_QUOTES, 'UTF-8') : ''; ?>">
                                    <span style="align-self: center;">:</span>
                                    <input type="text" name="var<?php echo $i; ?>" id="sms_var_<?php echo $i; ?>"
                                        placeholder="Value" style="flex: 1;"
                                        value="<?php echo isset($varValues[$i - 1]) ? htmlspecialchars($varValues[$i - 1], ENT_QUOTES, 'UTF-8') : ''; ?>">
                                </div>
                            </div>
                        <?php endfor; ?>
                        <div class="btn-group">
                    ...
                    <div class="tab-content" id="whatsapp">
                        <div class="condition-box">
                            ...
                            <select class="condition-select" id="mediaType" name="media_type">
                                <option value="image" <?php echo (isset($existingData['media_type']) && $existingData['media_type'] == 'image') ? 'selected' : ''; ?>>Image</option>
                                <option value="video" <?php echo (isset($existingData['media_type']) && $existingData['media_type'] == 'video') ? 'selected' : ''; ?>>Video</option>
                                <option value="text" <?php echo (!isset($existingData['media_type']) || $existingData['media_type'] == 'text') ? 'selected' : ''; ?>>Text</option>
                                <option value="pdf" <?php echo (isset($existingData['media_type']) && $existingData['media_type'] == 'pdf') ? 'selected' : ''; ?>>Pdf</option>
                            </select>
                        </div>
                        <div class="condition-box" id="mediaSourceBox">
                            <span>Media Source :</span>
                            <label>
                                <input type="radio" name="media_source_type" value="url" <?php echo (!isset($existingData['media_source']) || $existingData['media_source'] == 'input') ? 'checked' : ''; ?>> URL
                            </label>
                            <label>
                                <input type="radio" name="media_source_type" value="file" <?php echo (isset($existingData['media_source']) && $existingData['media_source'] == 'file') ? 'checked' : ''; ?>> Upload
                            </label>
                        ...
                        <div class="condition-box" id="mediaUrlBox">
                            <input type="text" name="media_url" class="condition-input" placeholder="Enter media url"
                                style="flex:1;"
                                value="<?php echo isset($existingData['media_url']) ? htmlspecialchars($existingData['media_url'], ENT_QUOTES, 'UTF-8') : ''; ?>">
                        </div>
                        ...
                        <div class="condition-box" id="mediaFileUrlBox" style="display:none;">
                            ...
                            <input type="text" name="generated_media_url" class="condition-input"
                                ...
                                value="<?php echo (isset($existingData['media_source']) && $existingData['media_source'] == 'file' && !empty($existingData['media_url'])) ? htmlspecialchars($existingData['media_url'], ENT_QUOTES, 'UTF-8') : ''; ?>">
                        </div>
                        <div id="buttons-container">
                            <?php
                            $buttonConfigs = array(
                                1 => array(
                                    'type' => isset($existingData['button_type']) ? $existingData['button_type'] : '',
                                    'text' => isset($existingData['button_text1_type']) ? $existingData['button_text1_type'] : '',
                                    'url' => ''
                                ),
                                2 => array(
                                    'type' => isset($existingData['button_type2']) ? $existingData['button_type2'] : '',
                                    'text' => isset($existingData['button_text2_type']) ? $existingData['button_text2_type'] : '',
                                    'url' => ''
                                ),
                                3 => array(
                                    'type' => isset($existingData['button_type3']) ? $existingData['button_type3'] : '',
                                    'text' => isset($existingData['button_text3_type']) ? $existingData['button_text3_type'] : '',
                                    'url' => ''
                                )
                            );

                            if (isset($existingData['cta_url'])) {
                                $ctaUrls = json_decode($existingData['cta_url'], true);
                                if (is_array($ctaUrls)) {
                                    if (isset($ctaUrls['button1']))
                                        $buttonConfigs[1]['url'] = $ctaUrls['button1'];
                                    if (isset($ctaUrls['button2']))
                                        $buttonConfigs[2]['url'] = $ctaUrls['button2'];
                                    if (isset($ctaUrls['button3']))
                                        $buttonConfigs[3]['url'] = $ctaUrls['button3'];
                                }
                            }
                            ?>
                            <?php for ($btnNum = 1; $btnNum <= 3; $btnNum++): ?>
                                <?php if ($btnNum == 1 || !empty($buttonConfigs[$btnNum]['type']) || !empty($buttonConfigs[$btnNum]['text'])): ?>
                                    <div class="button-group" id="button-group-<?php echo $btnNum; ?>">
                                        <div class="button-header">
                                            <span class="button-title">Button <?php echo $btnNum; ?></span>
                                            <?php if ($btnNum > 1): ?>
                                                <button type="button" class="remove-button-btn"
                                                    onclick="removeButton(<?php echo $btnNum; ?>)">
                                        ...
                                        <div class="button-row">
                                            <select name="button_type<?php echo $btnNum; ?>" class="button-type"
                                                data-button="<?php echo $btnNum; ?>"
                                                onchange="toggleUrlField(<?php echo $btnNum; ?>)">
                                                <option value="none" <?php echo ($buttonConfigs[$btnNum]['type'] == 'none' || $buttonConfigs[$btnNum]['type'] == '') ? 'selected' : ''; ?>>None</option>
                                                <option value="cta" <?php echo ($buttonConfigs[$btnNum]['type'] == 'cta') ? 'selected' : ''; ?>>CTA</option>
                                                <option value="quick" <?php echo ($buttonConfigs[$btnNum]['type'] == 'quick') ? 'selected' : ''; ?>>Quick Reply</option>
                                            </select>
                                            <input type="text" name="button_text<?php echo $btnNum; ?>"
                                                placeholder="Button Text"
                                                value="<?php echo htmlspecialchars($buttonConfigs[$btnNum]['text'], ENT_QUOTES, 'UTF-8'); ?>">
                                        </div>
                                        <div class="url-field" id="url-field-<?php echo $btnNum; ?>" <?php echo ($buttonConfigs[$btnNum]['type'] != 'cta') ? 'style="display:none;"' : ''; ?>>
                                            <input type="text" name="button_url<?php echo $btnNum; ?>"
                                                placeholder="Enter URL (e.g., https://example.com)"
                                                value="<?php echo htmlspecialchars($buttonConfigs[$btnNum]['url'], ENT_QUOTES, 'UTF-8'); ?>">
                                        </div>
                                    </div>
                                <?php endif; ?>
                            <?php endfor; ?>
                        ...
                        <div class="condition-box">
                            ...
                            <input type="text" name="whatsapp" class="condition-input" placeholder="Enter template name"
                                style="flex:1;" value="<?php
                                $whatsappData = array();
                                if (isset($existingData['whatsapp']) && !empty($existingData['whatsapp'])) {
                                    $whatsappData = json_decode($existingData['whatsapp'], true);
                                    if (is_array($whatsappData) && isset($whatsappData['template_name'])) {
                                        echo htmlspecialchars($whatsappData['template_name'], ENT_QUOTES, 'UTF-8');
                                    }
                                }
                                ?>">
                        </div>
                        <div class="variables-section">
                            ...
                            <div class="sub-section">
                                ...
                                <div id="headers-container">
                                    <?php
                                    $headers = array();
                                    if (isset($whatsappData['variable_headers']) && is_array($whatsappData['variable_headers'])) {
                                        $headers = $whatsappData['variable_headers'];
                                    }
                                    if (empty($headers)) {
                                        $headers = array('');
                                    }
                            ...
                            <div class="sub-section">
                                ...
                                <div id="body-container">
                                    <?php
                                    $bodyVars = array();
                                    if (isset($whatsappData['variable_body']) && is_array($whatsappData['variable_body'])) {
                                        $bodyVars = $whatsappData['variable_body'];
                                    }
                                    if (empty($bodyVars)) {
                                        $bodyVars = array('');
                                    }
                                    foreach ($bodyVars as $index => $body):
                                        ?>
            ...
            <div class="right-panel">
                <div class="variable-box">
                    ...
                    <div class="variable-list">
                        ...
                        <div class="variable-item">{{ order_cancel_reason }}</div>
    ...
    <script>
        var buttonCount = <?php
        $count = 1;
        if (!empty($buttonConfigs[2]['type']) || !empty($buttonConfigs[2]['text']))
            $count = 2;
        if (!empty($buttonConfigs[3]['type']) || !empty($buttonConfigs[3]['text']))
            $count = 3;
        echo $count;
        ?>;
        var headerCounter = <?php echo count($headers); ?>;
        var bodyCounter = <?php echo count($bodyVars); ?>;
