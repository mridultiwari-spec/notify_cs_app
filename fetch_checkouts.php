...
{
    try {
        $stmt = $pdo->prepare("SELECT session_access_token FROM $configTable WHERE shop = :shop LIMIT 1");
        $stmt->execute(array(':shop' => $shop));
        $result = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$result || empty($result['session_access_token'])) {
            return array('success' => false, 'error' => 'No session token found');
        }

        $sessionToken = $result['session_access_token'];

        $refreshResult = get_valid_shop_access_token_php53(
            $pdo,
            $configTable,
            $shop,
            $api_key,
        ...
        if ($refreshResult['success']) {
            return array('success' => true, 'access_token' => $refreshResult['access_token']);
        } else {
            return array('success' => false, 'error' => $refreshResult['error']);
        }
    } catch (Exception $e) {
...
{
    global $pdo, $prefix, $api_key, $api_secret;
...
';
        ...
        for ($retry = 0; $retry <= $max_retries; $retry++) {
            $response = shopifyGraphQLRequest($api_url, $current_token, $query);

            if ($response === false || (isset($response['errors']) && isTokenError($response))) {
                file_put_contents(
                    __DIR__ . "/file/debug_log.txt",
                    "Token error detected, attempting refresh for shop: $shop\n",
                    FILE_APPEND
                );

                $refreshResult = refreshShopAccessTokenForAbandonedCheckout($shop, $pdo, $configTable, $api_key, $api_secret);

                if ($refreshResult['success']) {
                    $current_token = $refreshResult['access_token'];
                    file_put_contents(
                        __DIR__ . "/file/debug_log.txt",
                        "Token refreshed successfully for shop: $shop\n",
                        FILE_APPEND
                    );
                    continue;
                } else {
                    file_put_contents(
                        __DIR__ . "/file/debug_log.txt",
                        "Token refresh failed for shop: $shop - " . $refreshResult['error'] . "\n",
                        FILE_APPEND
                    );
                    break;
                }
        ...
        foreach ($checkouts as $edge) {
            ...
            $checkout_id = '';
            echo "Processing checkout ID: " . (isset($checkout['id']) ? $checkout['id'] : 'NO ID') . "<br>\n";
            echo "completedAt: " . (isset($checkout['completedAt']) ? $checkout['completedAt'] : 'NULL (empty)') . "<br>\n";

            if (!empty($checkout['completedAt'])) {
                echo "SKIPPING: This checkout has completedAt value<br>\n";
                continue;
            }
            ...
            $checkout_data = array(
                ...
                'total_price' => isset($checkout['totalPriceSet']['shopMoney']['amount']) ? $checkout['totalPriceSet']['shopMoney']['amount'] : '0',
                'subtotal_price' => isset($checkout['subtotalPriceSet']['shopMoney']['amount']) ? $checkout['subtotalPriceSet']['shopMoney']['amount'] : '0',
                'total_tax' => isset($checkout['totalTaxSet']['shopMoney']['amount']) ? $checkout['totalTaxSet']['shopMoney']['amount'] : '0',
                'currency' => isset($checkout['totalPriceSet']['shopMoney']['currencyCode']) ? $checkout['totalPriceSet']['shopMoney']['currencyCode'] : '',
                'shipping_address' => json_encode(isset($checkout['shippingAddress']) ? $checkout['shippingAddress'] : array()),
                'billing_address' => json_encode(isset($checkout['billingAddress']) ? $checkout['billingAddress'] : array()),
        ...
        if ($total_fetched >= 2000) {
            file_put_contents(
                __DIR__ . "/file/debug_log.txt",
                "WARNING: 2000 checkout limit reached\n",
                FILE_APPEND
            );
            break;
        }
...
function isTokenError($response)
{
    ...
    return (stripos($errorText, 'Invalid API key or access token') !== false ||
        stripos($errorText, 'access token expired') !== false);
}
