...
function processAbandonedCheckoutNotifications($shop, $oauth_token)
{
    ...
    file_put_contents($logFile, " - Starting abandoned checkout notification processing for shop: $shop\n", FILE_APPEND);
    ...
    $sms_enabled_check = isset($notification_config['sms_enabled']) ? (int) $notification_config['sms_enabled'] : 0;
    $whatsapp_enabled_check = isset($notification_config['whatsapp_enabled']) ? (int) $notification_config['whatsapp_enabled'] : 0;

    if ($sms_enabled_check == 0 && $whatsapp_enabled_check == 0) {
        file_put_contents($logFile, "Both SMS and WhatsApp are disabled for abandoned checkouts. Skipping.\n", FILE_APPEND);
        return array('processed' => 0, 'sms_sent' => 0, 'whatsapp_sent' => 0);
    }
    ...
    file_put_contents(
        $logFile,
        " - Completed processing | Shop: $shop | " .
...
function sendCheckoutNotification($checkout, $config, $pdo, $checkout_table, $log_table, $shop)
{
    ...
    if (count($line_items) > 0) {
        ...
        foreach ($line_items as $item) {
            if (isset($item['total_discount'])) {
                $discountSum += floatval($item['total_discount']);
            }
        }
        $total_discount = number_format($discountSum, 2, '.', '');
    }
    ...
    file_put_contents($logFile, " - Processing checkout $checkout_id for customer: $customer_full_name\n", FILE_APPEND);
    ...
    file_put_contents(
        $logFile,
        " - Checkout $checkout_id processed | " .
...
function updateCheckoutStatus($pdo, $table, $id, $status)
{
    try {
        ...
        global $logFile;
        file_put_contents($logFile, " ERROR updating status: " . $e->getMessage() . "\n", FILE_APPEND);
        return false;
    }
}
