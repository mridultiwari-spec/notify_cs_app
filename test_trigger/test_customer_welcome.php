...
try {
    ...
    if ($customer === null) {
        ...
        echo json_encode(array('success' => false, 'message' => 'Failed to parse customer JSON: ' . $json_error));
        exit;
    }
    ...
    if ($default_address && isset($default_address->country_code)) {
        $default_address_country_code = strtoupper($default_address->country_code);
    }
    ...
    if (!$row) {
        echo json_encode(array('success' => false, 'message' => 'Template not found for Customer Welcome (aid=10)'));
        exit;
    }
...
if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
    error_log("Processing WhatsApp for customer welcome test - Template: $whatsapp_template_name");
    ...
    if (empty($final_phone_number)) {
        error_log("ERROR: Customer phone is empty, cannot send WhatsApp for customer welcome test");
    } else {
        if (function_exists('send_whatsapp_message')) {
            $whatsapp_result = send_whatsapp_message($whatsapp_config, $pdo, $whatsapp_table2);

            if ($whatsapp_result['success']) {
                error_log("WhatsApp sent successfully for customer welcome test to {$final_phone_number}");
            } else {
                error_log("WhatsApp failed for customer welcome test: " . $whatsapp_result['message']);
            }
        } else {
    ...
    error_log("No WhatsApp template configured for aid=10, skipping WhatsApp send");
}
?>