...
$notification_type = "Time Period Based";
...
try {
    ...
    $created_at = isset($order_details->created_at) ? $order_details->created_at : '';
...
if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
    error_log("Processing WhatsApp for time period based test - Template: $whatsapp_template_name");
    ...
    if (empty($customer_phone)) {
        error_log("ERROR: Customer phone is empty, cannot send WhatsApp for time period based test");
    } else {
        if (function_exists('send_whatsapp_message')) {
            $whatsapp_result = send_whatsapp_message($whatsapp_config, $pdo, $whatsapp_table2);

            if ($whatsapp_result['success']) {
                error_log("WhatsApp sent successfully for time period based test to {$customer_phone}");
            } else {
                error_log("WhatsApp failed for time period based test: " . $whatsapp_result['message']);
            }
        } else {
