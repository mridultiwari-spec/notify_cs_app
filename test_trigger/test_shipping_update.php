...
try {
    ...
    $shipment_status = isset($fulfillment->shipment_status) ? $fulfillment->shipment_status : '';
    ...
    if (!$row) {
        echo json_encode(array('success' => false, 'message' => 'Template not found for Shipping Update (aid=8)'));
        exit;
    }
    ...
    if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
        ...
        if (empty($final_phone_number)) {
            error_log("ERROR: Customer phone is empty, cannot send WhatsApp for shipping update test");
        } else {
            if (function_exists('send_whatsapp_message')) {
                $whatsapp_result = send_whatsapp_message($whatsapp_config, $pdo, $whatsapp_table2);
                
                if ($whatsapp_result['success']) {
                    error_log("WhatsApp sent successfully for shipping update test to {$final_phone_number}");
                } else {
                    error_log("WhatsApp failed for shipping update test: " . $whatsapp_result['message']);
                }
            } else {
        ...
        error_log("No WhatsApp template configured for aid=8, skipping WhatsApp send");
    } 
} catch (Exception $e) {
