...
$notification_type = "Order Value Based";
...
if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
    error_log("Processing WhatsApp for order value based test - Template: $whatsapp_template_name");
    ...
    if (empty($customer_phone)) {
        error_log("ERROR: Customer phone is empty, cannot send WhatsApp for order value based test");
    } else {
        if (function_exists('send_whatsapp_message')) {
            $whatsapp_result = send_whatsapp_message($whatsapp_config, $pdo, $whatsapp_table2);

            if ($whatsapp_result['success']) {
                error_log("WhatsApp sent successfully for order value based test to {$customer_phone}");
            } else {
                error_log("WhatsApp failed for order value based test: " . $whatsapp_result['message']);
            }
        } else {
