...
require_once __DIR__ . '/../accurate_country_code.php';
require_once __DIR__ . '/../send_sms_api.php';
...
if (!$recordId) {
    echo json_encode(array('success' => false, 'message' => 'Missing record ID'));
    exit;
}
...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM $table WHERE id = :id AND shop = :shop");
    $stmt->execute(array(':id' => $recordId, ':shop' => $shop));
    $row = $stmt->fetch();
    if (!$row) {
    ...
    $product_tags = isset($order_details->tags) ? $order_details->tags : '';
    ...
    if ($sms_enabled == 1) {
        ...
        if ($has_parameters) {
            ...
            if (empty($customer_phone)) {
                echo json_encode(array('success' => false, 'message' => 'Customer phone is empty'));
                exit;
            } elseif (empty($template_id)) {
                echo json_encode(array('success' => false, 'message' => 'Template ID is empty'));
                exit;
            } else {
                ...
                echo json_encode(array('success' => true, 'message' => 'Test SMS sent successfully using template'));
            }
        } else {
            $searchVal = array(
                "{{ order_name }}",
            ...
            if (empty($customer_phone)) {
                ...
                echo json_encode(array('success' => false, 'message' => 'SMS text is empty'));
                exit;
            } else {
        ...
        // SMS is disabled
        echo json_encode(array('success' => true, 'message' => 'SMS is disabled for this template, skipping SMS', 'sms_enabled' => false));
    }
...
// ========== WHATSAPP MESSAGING FOR PRODUCT TAGS BASED TEST ==========
...
if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
    error_log("Processing WhatsApp for product tags based test - Template: $whatsapp_template_name");
    ...
    $wa_phone = "$countryCode" . "$customer_phone";
    ...
    if (empty($customer_phone)) {
        error_log("ERROR: Customer phone is empty, cannot send WhatsApp for product tags based test");
    } else {
        if (function_exists('send_whatsapp_message')) {
            $whatsapp_result = send_whatsapp_message($whatsapp_config, $pdo, $whatsapp_table2);

            if ($whatsapp_result['success']) {
                error_log("WhatsApp sent successfully for product tags based test to {$customer_phone}");
            } else {
                error_log("WhatsApp failed for product tags based test: " . $whatsapp_result['message']);
            }
        } else {
    ...
    error_log("No WhatsApp template configured for record id=$recordId, skipping WhatsApp send");
}
?>