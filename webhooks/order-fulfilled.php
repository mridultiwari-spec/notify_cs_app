...
if (isset($fulfillment->fulfillments) && !empty($fulfillment->fulfillments)) {
    $last_fulfillment = end($fulfillment->fulfillments);
    $tracking_number = isset($last_fulfillment->tracking_number) ? $last_fulfillment->tracking_number : '';
...
$billing_address = isset($fulfillment->billing_address) ? $fulfillment->billing_address : null;
$billing_string = $billing_address ? implode(', ', array_filter(array(
    ...
    isset($billing_address->company) ? $billing_address->company : '',
    isset($billing_address->city) ? $billing_address->city : '',
    isset($billing_address->province) ? $billing_address->province : '',
    isset($billing_address->zip) ? $billing_address->zip : '',
    isset($billing_address->country) ? $billing_address->country : '',
    isset($billing_address->phone) ? $billing_address->phone : ''
))) : '';

$shipping_city = ($destination && isset($destination->city)) ? $destination->city : '';
$shipping_state = ($destination && isset($destination->province)) ? $destination->province : '';
...
$fulfillment_root_phone = isset($fulfillment->phone) ? $fulfillment->phone : '';

$shipping_country_code = $destination_country_code;

if ($shipping_country_code) {
    $country_code = strtoupper($shipping_country_code);
} elseif ($destination_country_code) {
    $country_code = strtoupper($destination_country_code);
} else {
    $country_code = 'IN';
}
...
} elseif ($fulfillment_root_phone) {
    $customer_phone = $fulfillment_root_phone;
...
$subject = ucwords("Shipping Confirmation");

$prefix = $app_prefix;
$table = $prefix . "shopify_sms_notification_App_Email_Notification";
$stmt = $pdo->prepare("SELECT * FROM $table WHERE aid = 7 AND shop = :shop");
$stmt->execute(array(':shop' => $shop));
$row = $stmt->fetch();
...
if ($row) {
    ...
    if ($sms_enabled == 1) {
        ...
        if ($has_parameters) {
            ...
            if (empty($customer_phone)) {
                ...
                file_put_contents("$logFile", "ERROR: Template ID is empty, cannot send template SMS for fulfillment {$fulfillment_id}\n", FILE_APPEND);
            } else {
                if (function_exists('send_smstext_with_parameters')) {
                    ...
                    send_smstext($country_code, $customer_phone, $template_name_sms, $shop, $customer_email_id, $customer_full_name, $order_id, $order_name, $subject);
            ...
            $searchVal[] = "{{ order_total_price }}";
            $replaceVal[] = isset($fulfillment->total_price) ? $fulfillment->total_price : '';
            ...
            if (empty($final_text) || empty($sms_text)) {
                $final_text = "Your order {$order_name} has been shipped! Tracking: {$tracking_number}";
                file_put_contents("$logFile", "WARNING: Using fallback SMS text for fulfillment\n", FILE_APPEND);
            }

            $searchReplaceMap = array(
                '{{ order_name }}' => isset($order_name) ? $order_name : null,
                '{{ Ad_order_number }}' => isset($order_id) ? $order_id : null,
                '{{ tracking_number }}' => isset($tracking_number) ? $tracking_number : null,
                '{{ tracking_url }}' => isset($tracking_url) ? $tracking_url : null,
                ...
                '{{ shipping_address }}' => isset($shipping_string) ? $shipping_string : null,
    ...
    file_put_contents("$logFile", "WARNING: No enabled SMS template found for aid=7 and shop={$shop}\n", FILE_APPEND);
}
...
if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
    file_put_contents("$logFile", "Processing WhatsApp for order fulfilled - Template: $whatsapp_template_name\n", FILE_APPEND);
    ...
    $replacement_map['order_total_price'] = isset($fulfillment->total_price) ? $fulfillment->total_price : '';
    ...
    if (empty($customer_phone)) {
        ...
        if ($whatsapp_result['success']) {
            file_put_contents("$logFile", "WhatsApp sent successfully for order fulfilled {$fulfillment_id} to {$customer_phone}\n", FILE_APPEND);
        } else {
            file_put_contents("$logFile", "WhatsApp failed for fulfillment {$fulfillment_id}: " . $whatsapp_result['message'] . "\n", FILE_APPEND);
        }
    }
} else {
    file_put_contents("$logFile", "No WhatsApp template configured for aid=7, skipping WhatsApp send\n", FILE_APPEND);
}
?>