...
if ($row) {
    ...
    if ($sms_enabled == 1) {
        ...
        if ($has_parameters) {
            ...
            if (empty($customer_phone)) {
                ...
                file_put_contents("$logFile", "ERROR: Template Name is empty, cannot send template SMS for order {$order_id}\n", FILE_APPEND);
            } else {
                send_smstext_with_parameters(
                    ...
                    $customer_email_id,
                    $customer_full_name,
                    $parameter_values,  // Parameter values array
                    $order_id,
                    $order_name,
                    $notification_type
                );
                file_put_contents("$logFile", "Template SMS sent successfully for order cancellation {$order_id} to {$customer_phone}\n", FILE_APPEND);
            ...
            if (empty($final_text) || empty($sms_text)) {
                $final_text = "Your order {$order_name} has been cancelled. Reason: {$order_cancel_reason}";
                file_put_contents("$logFile", "WARNING: Using fallback SMS text for order cancellation\n", FILE_APPEND);
            }
            ...
            $searchReplaceMap = array(
                ...
                '{{ order_cancel_reason }}' => isset($order_cancel_reason) ? $order_cancel_reason : null,
            );
            ...
            // Send SMS using plain text function
            if (empty($customer_phone)) {
                file_put_contents("$logFile", "ERROR: Customer phone is empty, cannot send SMS for order {$order_id}\n", FILE_APPEND);
            } elseif (empty($template_name_sms)) {
                file_put_contents("$logFile", "ERROR: Template Name is empty, cannot send template SMS for order {$order_id}\n", FILE_APPEND);
    ...
    file_put_contents("$logFile", "WARNING: No enabled SMS template found for aid=3 and shop={$shop}\n", FILE_APPEND);
}

file_put_contents(
    "$logFile",
    "Cancel Reason: $order_cancel_reason\nDiscount: $total_discount\nBilling: $billing_string\nShipping: $shipping_string\nOrder_Status_Url: $order_status_url\n\n",
    FILE_APPEND
);
...
if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
    file_put_contents("$logFile", "Processing WhatsApp for order cancelled - Template: $whatsapp_template_name\n", FILE_APPEND);
    ...
    if (empty($customer_phone)) {
        ...
        if ($whatsapp_result['success']) {
            file_put_contents("$logFile", "WhatsApp sent successfully for order cancelled {$order_id} to {$customer_phone}\n", FILE_APPEND);
        } else {
            file_put_contents("$logFile", "WhatsApp failed for order {$order_id}: " . $whatsapp_result['message'] . "\n", FILE_APPEND);
        }
    }
} else {
    file_put_contents("$logFile", "No WhatsApp template configured for aid=3, skipping WhatsApp send\n", FILE_APPEND);
}
?>