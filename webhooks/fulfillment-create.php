...
$subject = ucwords("Fulfillment Request");

$prefix = $app_prefix;
$table = $prefix . "shopify_sms_notification_App_Email_Notification";
$stmt = $pdo->prepare("SELECT * FROM $table WHERE aid = 6 AND shop = :shop");
$stmt->execute(array(':shop' => $shop));
$row = $stmt->fetch();
...
if ($row) {
    ...
    if ($sms_enabled == 1) {
        ...
        if ($has_parameters) {
            ...
            file_put_contents("$logFile", "========== FULFILLMENTS-CREATE SMS ==========\n", FILE_APPEND);
    ...
    file_put_contents("$logFile", "WARNING: No SMS template found for aid=6 and shop={$shop}\n", FILE_APPEND);
}

file_put_contents(
    "$logFile",
    "Fulfillment Created - ID: $fulfillment_id\nOrder ID: $order_id\nTracking: $tracking_number\nTracking URL: $tracking_url\nShipping City: $shipping_city\nShipping State: $shipping_state\nShipping: $shipping_string\nCustomer Phone: $customer_phone\nCountry Code: $country_code\nSMS Text: " . (isset($final_text) ? $final_text : '') . "\n\n",
    FILE_APPEND
);
set_http_status(200);
...
if ($whatsapp_enabled == 1 && !empty($whatsapp_template_name)) {
    file_put_contents("$logFile", "Processing WhatsApp for fulfillment created - Template: $whatsapp_template_name\n", FILE_APPEND);
    ...
    if (empty($customer_phone)) {
        ...
        if ($whatsapp_result['success']) {
            file_put_contents("$logFile", "WhatsApp sent successfully for fulfillment {$fulfillment_id} to {$customer_phone}\n", FILE_APPEND);
        } else {
            file_put_contents("$logFile", "WhatsApp failed for fulfillment {$fulfillment_id}: " . $whatsapp_result['message'] . "\n", FILE_APPEND);
        }
    }
} else {
    file_put_contents("$logFile", "No WhatsApp template configured for aid=6, skipping WhatsApp send\n", FILE_APPEND);
}
